(function ($) {
	...
	$(document).ready(function () {
		...
		function positionSearch() {
			...
			var top =
				position.top +
				height / 2 -
				$(".search-overlay").outerHeight() / 2;
			var left =
				position.left -
				width / 2 -
				$(".search-overlay").outerWidth() / 2;
		...
		function horizontalSections() {
			...
			$(".sections-wrapper")
				.css("width", vWidth * count)
				.css(
					"height",
					$(".sections-wrapper section.active").outerHeight()
				);
		}
		...
		function slide($type, $this) {
			...
			$('.main-nav a[href="#section' + url + '"]')
				.parent()
				.addClass("active");
			$('.responsive-nav a[href="#section' + url + '"]')
				.parent()
				.addClass("active");
		...
		$(
			"a.forward, .section-nav a.backward, .main-nav a, .responsive-nav a, .available"
		).smoothScroll();
	...
	$(".contact-form").on("submit", function () {
		...
		if (
			contactForm.find(".contact-email").val().length !== 0 &&
			...
			let phoneNumber = contactForm.find(".contact-phone").val();
			let email = contactForm.find(".contact-email").val();
			let message = contactForm.find(".contact-message").val();
			let messageFormat = `Hola Eliezer has recibido un mensaje de alguien interesado en tus servicios.
			\nNombre: ${name} 
...
Email: ${email}
			...
			action = "https://api.ultramsg.com/instance43969/messages/chat";
			let data = JSON.stringify({
				token: "0fqidlow330tfr8i",
				to: "+584127592499",
				body: messageFormat,
			});
			fetch("https://api.ultramsg.com/instance43969/messages/chat", {
				method: "POST",
				headers: {
				...
				.then((data) => {
					...
					if (data.sent) {
						...
						contactForm
							...
							.find(".message")
							.html("Mensaje Enviado Extiosamente!");
				...
				.catch((error) => {
					...
					contactForm
						.find(".contact-error")
						.find(".message")
						.html("Perdon, ha ocurrido un error.");
			...
			// 			.find(".contact-success")
			// 			.find(".message")
			// 			.html("Success! Thanks for contacting us!");
			// 		contactForm.find(".contact-success").fadeIn();
			// 	},
			// 	error: function () {
			// 		contactForm.find(".contact-loading").fadeOut();
